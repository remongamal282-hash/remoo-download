...
التصميم النهائي يفصل `remo-service` و`remo-gui` كعمليتين مستقلتين. لكن بنية المشروع الحالية في `CMakeLists.txt` تبني هدفًا تنفيذيًا واحدًا باسم `RemooDownload`. لذلك يتم اعتماد مسار مرحلي:
